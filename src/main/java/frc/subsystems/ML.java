...
public class ML extends SubsystemBase {
    ...
    public ML() {
        ...
        // camera = NetworkTableInstance.getDefault().getTable("ML"+
        // NetworkTable.PATH_SEPARATOR+"MLOut");
    ...
    public double movement() {
        ...
        // because length is a multiple of 4, and divsions round down, this will end up
        // being 1/4 the length of
        // the `inputCoords` array
        BoundingBox[] boxes = new BoundingBox[inputCoords.length / 4];

        // loop through boxes starting at zero and going up to the total length of the
        // array
        for (int i = 0; i < boxes.length; i++) {
            ...
            boxes[i] = new BoundingBox(inputCoords[(i * 4) + 0], inputCoords[(i * 4) + 1], inputCoords[(i * 4) + 2],
                    inputCoords[(i * 4) + 3]);
        }
        // find max bounding box by area, therefore being the closet. Theoretically
        // something could be closer if

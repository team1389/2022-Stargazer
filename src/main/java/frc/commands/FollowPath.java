...
public class FollowPath extends CommandBase {
  ...
  //The array of positions in the form (x, y) marking the desired path. x positive is left. Angles in degrees CCW. Distance in meters.
  ...
  private Trajectory calculateTrajectory(double[][] points, double startAngle, double endAngle, double startVelocity, double endVelocity) {
    Pose2d startPose = new Pose2d(points[0][0], points[0][1], Rotation2d.fromDegrees(startAngle));
    Pose2d endPose = new Pose2d(points[points.length-1][0], points[points.length-1][1], Rotation2d.fromDegrees(endAngle));

    ArrayList<Translation2d> path = new ArrayList<>();

    //If the desired set of points contains at least one waypoint, add them to the path object
    if(points.length > 2) {
